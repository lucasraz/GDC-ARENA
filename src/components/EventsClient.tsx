...
function EventCard({ event, userId, onJoin, onLeave, onDelete, onTogglePayment, sendWhatsAppMessage }: any) {
    ...
    const [selectedBeer, setSelectedBeer] = useState('nenhuma')
    const [guestInput, setGuestInput] = useState({ name: '', beer: 'nenhuma', isAdding: false })
    ...
    event.attendees?.forEach((a: any) => {
        ...
        if (!a.is_paid) {
            const bp = availableBeers.find(b => b.id === a.selected_beer)?.price || 0
            groupedAttendees[a.user_id].totalPending += ((event.price || 0) + bp)
        }
    })
                            ...
                            <div style={{ 
                                                ...
                                                return (
                                                    <div key={r.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '0.8rem', gap: '0.5rem' }}>
                                                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', minWidth: 0 }}>
                                                            <Trash2 
                                                                size={14} 
                                                                onClick={(e) => { e.stopPropagation(); onLeave(r.id); }} 
                                                                style={{ 
                                                                    color: '#E57373', 
                                                                    cursor: 'pointer',
                                ...
                                {!userId ? (
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                                        ...
                                        <button onClick={(e) => { e.stopPropagation(); setGuestInput({...guestInput, isAdding: true })}} className="btn-primary" style={{ background: 'transparent', border: '1px solid var(--primary)', color: 'var(--primary)', padding: '0.8rem' }}>+ CONVIDADO</button>
                                        ...
                                        return (
                                            <div key={`${event.id}-${attendeeUserId}`} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.4rem 0.75rem', background: 'var(--surface-container-high)', borderRadius: '4px', gap: '1rem' }}>
                                                ...
                                                <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', marginLeft: 'auto' }}>
                                                    <button 
                                                        onClick={(e) => { e.stopPropagation(); onTogglePayment(attendeeUserId, !isAllPaid, event, group.profile, group.records); }}
                                                        style={{ 
...
export default function EventsClient({ userId, tenantId, initialEvents }: any) {
  ...
  const handleJoin = async (eventId: string, guestName?: string, beer?: string) => {
    ...
    const result = await joinEvent(eventId, userId, beer || 'nenhuma', guestName)
    if (result.success) {
      router.refresh()
    } else {
      alert(result.error)
